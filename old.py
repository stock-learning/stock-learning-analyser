...
def training(stock_name='bvsp_train', _epochs=1000, _batch_size=128, _verbose=0, _optimizer='adam', _loss='mean_squared_error'):
    _data = functions.load_dataframe(stock_name)
    ...
    _model.save(f'models/model_{functions.get_stock_name(stock_name)}')

def production(stock_name='bvsp_test'):
    _data = functions.load_dataframe(stock_name)
    _data = functions.apply_standardization(_data)
    _test_x, _test_y = functions.get_only_normalized_values(_data, return_real_value=True, to_numpy=False)
    _model = neural_network.load(functions.get_stock_name(stock_name))
